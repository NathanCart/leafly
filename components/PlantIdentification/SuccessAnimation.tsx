...
export function SuccessAnimation({ onAnimationComplete, title }: Props) {
	...
	const textStyle = useAnimatedStyle(() => ({
		...
		transform: [{ scale: interpolate(textOpacity.value, [0, 1.2, 1], [0.8, 1.2, 1]) }],
	}));
	...
	const Confetti = ({ style, delay = 0 }) => {
		...
		return <Animated.View style={[styles.confetti, style, confettiStyle]} />;
	};
	...
	const Sparkle = ({ style, delay = 0 }) => {
		...
		return <Animated.View style={[styles.sparkle, style, sparkleAnimStyle, sparkleStyle]} />;
	};
